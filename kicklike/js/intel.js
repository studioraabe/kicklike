(() => {
  ...
  function buildMatchupIntel(squad, opp) {
    ...
    const { teamTotalPower, aggregateTeamStats, teamStatsEngineAligned } = KL.stats;
    ...
    // v0.49 — Scorecard-Stat-Vergleich jetzt engine-aligned statt plain
    // average. Die Engine nutzt rollen-gewichtete Kombinationen (ST-
    // heavy für offense, VT+TW für defense, etc.), und opp.stats ist
    // bereits team-aggregiert — der alte Avg-Approach verglich Apples
    // mit Oranges und zeigte eine viel größere Lücke an als real in
    // der Engine berechnet wird. User-Feedback: "schon in den ersten
    // games haben opps teilweise viel höhere attribute" — das lag
    // partly daran. Jetzt ist das Preview-Display konsistent mit dem
    // was die Engine tatsächlich gegen opp.stats ansetzt.
    const myAgg = teamStatsEngineAligned(squad);
  ...
  function estimateWinProbability(squad, opp, opts = {}) {
    if (!squad?.length || !opp) return null;
    // v0.49 — hier ebenfalls engine-aligned Stats (siehe buildMatchup-
    // Intel). Der alte aggregateTeamStats-Approach warf plain averages
    // (~50) gegen opp-Team-Aggregate (~100-130) in die ATTACK/DEF-
    // Formel, mit Delta -50..-80 und clamp auf 5% Scoring-Chance. Damit
    // war die Pre-Match-Win-Probability systematisch zu pessimistisch.
    const { teamStatsEngineAligned } = KL.stats;
    const my = teamStatsEngineAligned(squad);
