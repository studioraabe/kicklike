...
(() => {
  ...
  // Wie t(), aber gibt `fallback` zurück, wenn der Key nicht existiert.
  // Der normale t() liefert in dem Fall den Path-String als Debug-Signal,
  // was das `I18N.t(key) || fallback`-Pattern bricht (truthy string). Diese
  // Variante prüft explizit und greift auf den Fallback zurück.
  function tOr(path, fallback, vars = {}) {
    const value = resolvePath(path);
  ...
  // v52.6 — anti-repetition memory. Per-path Set of recently-used indices.
  // pickText avoids picking the same index twice in a row (and for pools of
  // 6+ entries, avoids the last 2). Drastically reduces "I keep seeing the
  // same line" feeling without needing to expand every pool. Reset on
  // language switch (locale change wipes the pool keys anyway).
  const _recentPicks = Object.create(null);

  function pickText(path, vars = {}) {
    const options = list(path);
    if (!options.length) return path;
    if (options.length === 1) return format(options[0], vars);

    // Rolling window of recent indices: 1 for tiny pools, 2 for medium+.
    // Never lock out more than half the pool (would deadlock pickText).
    const windowSize = Math.min(
      options.length - 1,
      options.length >= 6 ? 2 : 1
    );

    const recent = _recentPicks[path] || [];
    let idx;
    let attempts = 0;
    do {
      idx = Math.floor(Math.random() * options.length);
      attempts++;
    } while (recent.includes(idx) && attempts < 8);

    // Update rolling window (push idx, trim to windowSize)
    recent.push(idx);
    while (recent.length > windowSize) recent.shift();
    _recentPicks[path] = recent;

    return format(options[idx], vars);
  }
  ...
  function setLang(nextLang) {
    ...
    // v52.6 — wipe the anti-repetition memory: pool contents change with
    // language, so cached "recent index" entries from German pools would
    // be meaningless against English pools.
    for (const k of Object.keys(_recentPicks)) delete _recentPicks[k];
    applyDom();
