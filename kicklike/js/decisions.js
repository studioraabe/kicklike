...
// ─── FRAME_EVENTS — inline situation banners for the card-phase UI ───────
// Replaces the modal-based SITUATIVE_EVENTS when CONFIG.cardsEnabled is on.
// A frame is presented at the top of the card panel with:
//   - title + narrative text
//   - an automatic mechanical effect (applied on fire, no user choice)
//   - a hint about which card type addresses it
// The player's response IS the card play itself — no separate modal.
//
// Curated to 5 events that each deliver a distinct feel:
//   striker_frustrated  — own malus to resolve
//   keeper_in_zone      — own bonus to exploit
//   hot_corridor        — tactical read, payoff for lane cards
//   opp_star_down       — opportunity window, reward aggression
//   red_card_risk       — drama, cost for aggressive play this round
// ────────────────────────────────────────────────────────────────────────
...
const FRAME_EVENTS = [
  {
    ...
    condition(match) {
      ...
      if (!st || !match.memory) return false;
    ...
    resolveContext(match) {
      const st = match.squad.find(p => p.role === 'ST');
      const mem = match.memory.myPlayerStates?.[st.id] || {};
      return { player: st, streakCount: mem.consecutiveMisses || 0 };
    },
    frame(match, ctx) {
      ...
      return {
        icon:     '😤',
        severity: 'warn',
        title:    I18N.t('ui.frames.strikerFrustrated.title'),
        text:     I18N.t('ui.frames.strikerFrustrated.text', { name, n: ctx.streakCount }),
        effect:   I18N.t('ui.frames.strikerFrustrated.effect'),
        hint:     I18N.t('ui.frames.strikerFrustrated.hint')
      };
    },
    autoEffect(match, ctx) {
      // Level 1: -6 OFF. Level 2: -12 OFF + team composure -2 (the
      // striker is visibly tilted, whole team is affected). The
      // frame-level escalation rewards players who ignored the first
      // warning with a bigger problem on re-fire.
      const level = ctx._frameLevel || 1;
      const offPenalty = level >= 2 ? -12 : -6;
      match.buffLayers.push({
        source: 'frame:striker_frustrated',
        range:  [match.round, match.round],
        stats:  { offense: offPenalty },
        special: null
      });
      if (level >= 2) {
        match.buffLayers.push({
          source: 'frame:striker_frustrated_L2_team',
          range:  [match.round, match.round],
          stats:  { composure: -2 },
          special: null
        });
...

  {
    ...
    condition(match) {
      ...
      if (!tw || !match.memory) return false;
    ...
    resolveContext(match) {
      const tw = match.squad.find(p => p.role === 'TW');
      const mem = match.memory.myPlayerStates?.[tw.id] || {};
      return { player: tw, streakCount: mem.savesInRow || 0 };
    },
    frame(match, ctx) {
      const name = ctx.player?.name || '';
      return {
        icon:     '🧤',
        severity: 'good',
        title:    I18N.t('ui.frames.keeperInZone.title'),
        text:     I18N.t('ui.frames.keeperInZone.text', { name, n: ctx.streakCount }),
        effect:   I18N.t('ui.frames.keeperInZone.effect'),
        hint:     I18N.t('ui.frames.keeperInZone.hint')
      };
    },
    autoEffect(match, ctx) {
      // Level 1: +10 DEF, +4 TMP, +12 save. Level 2 (rare — keeper
      // stays hot an extra stretch): amped further. Good thing
      // stacking on good thing.
      const level = ctx._frameLevel || 1;
      const defBoost = level >= 2 ? 16 : 10;
      const tempoBoost = level >= 2 ? 6 : 4;
      const saveBoost = level >= 2 ? 18 : 12;
      match.buffLayers.push({
        source: 'frame:keeper_in_zone',
        range:  [match.round, match.round],
        stats:  { defense: defBoost, tempo: tempoBoost },
        special: null
      });
      match.nextSaveBonus = (match.nextSaveBonus || 0) + saveBoost;
    }
  },

  {
    ...
    fireChance: 0.55,
    condition(match) {
      // Fires when the engine has recorded 2+ successful builds on our side
      // OR when tempo differential favours us meaningfully.
      const s = match.stats || {};
      if ((s.myBuildupsSuccess || 0) >= 2) return true;
      return false;
    },
    resolveContext(match) {
    ...
    frame(match, ctx) {
      ...
      return {
        icon:     '🎯',
        severity: 'good',
        title:    I18N.t('ui.frames.hotCorridor.title'),
        text:     I18N.t('ui.frames.hotCorridor.text', { name }),
        effect:   I18N.t('ui.frames.hotCorridor.effect'),
        hint:     I18N.t('ui.frames.hotCorridor.hint')
      };
    },
    autoEffect(match, ctx) {
      // Opens a lane for free — the card layer's existing Lane Open
      // mechanic handles it. Any LaneOpen-consumer card cashes in.
      match._cardLaneOpen = true;
      match.buffLayers.push({
        source: 'frame:hot_corridor',
        range:  [match.round, match.round],
        stats:  { tempo: 5, offense: 4 },
        special: null
      });
    }
  },

  {
    ...
    fireChance: 0.50,
    condition(match) {
      // Fires when opp has conceded 2+ or lost momentum decisively.
      const s = match.stats || {};
      if (match.scoreMe - match.scoreOpp >= 2) return true;
      if ((s.oppBuildups || 0) >= 3 && (s.oppBuildupsSuccess || 0) <= 1) return true;
      return false;
    },
    resolveContext(match) {
      return { oppName: match.opp?.name || '' };
    },
    frame(match, ctx) {
      return {
        icon:     '⚡',
        severity: 'opportunity',
        title:    I18N.t('ui.frames.oppStarDown.title'),
        text:     I18N.t('ui.frames.oppStarDown.text', { opp: ctx.oppName }),
        effect:   I18N.t('ui.frames.oppStarDown.effect'),
        hint:     I18N.t('ui.frames.oppStarDown.hint')
      };
    },
    autoEffect(match, ctx) {
      // Opp is rattled — their next trait fire is dampened, and we get
      // tempo/offense for exploiting it.
      match.buffLayers.push({
        source: 'frame:opp_star_down',
        range:  [match.round, match.round],
        stats:  { tempo: 6, offense: 8 },
        special: null
      });
...

  {
    ...
    fireChance: 0.40,
    condition(match) {
      // Fires if any of our field players already carries a yellow.
      const squad = match.squad || [];
      return squad.some(p => p.role !== 'TW' && (p._yellowsThisMatch || 0) >= 1);
    },
    resolveContext(match) {
      const risky = (match.squad || [])
        .filter(p => p.role !== 'TW' && (p._yellowsThisMatch || 0) >= 1);
      // Prefer the one with the highest press/tempo (most likely to earn
      // a second yellow by fouling).
      risky.sort((a, b) => (b.stats?.tempo || 0) - (a.stats?.tempo || 0));
      return { player: risky[0] };
    },
    frame(match, ctx) {
      const name = ctx.player?.name || '';
      return {
        icon:     '🟨',
        severity: 'danger',
        title:    I18N.t('ui.frames.redCardRisk.title'),
        text:     I18N.t('ui.frames.redCardRisk.text', { name }),
        effect:   I18N.t('ui.frames.redCardRisk.effect'),
        hint:     I18N.t('ui.frames.redCardRisk.hint')
      };
    },
    autoEffect(match, ctx) {
      // No passive stat change — the drama is that aggressive cards this
      // round MIGHT send the at-risk player off. The card layer can read
      // _frameRedCardRiskPlayer and apply a chance on aggressive play.
      match._frameRedCardRiskPlayer = ctx.player?.id || null;
      match.buffLayers.push({
        source: 'frame:red_card_risk',
        range:  [match.round, match.round],
        stats:  { composure: -4 },
        special: null
      });
  ...
  // ── Opp-focused frames: opportunities to exploit the opposition ─────
  //
  // Opp keeper shaky — their TW has conceded several in a row AND we've
  // already put up shots. Read: their keeper's confidence is gone. Good
  // window for a quick trigger or combo. Mechanical: we get an OFF buff
  // and their next save attempt is dampened.
  {
    id: 'opp_keeper_shaky',
    window: [3, 6],
    priority: 6,
    fireChance: 0.55,
    condition(match) {
      // We've scored 2+ AND had more shots than saves — their keeper
      // feels beaten.
      const s = match.stats || {};
      const shots = s.myShots || 0;
      return match.scoreMe >= 2 && shots >= 3;
    },
    resolveContext(match) {
      return { oppName: match.opp?.name || '' };
    },
    frame(match, ctx) {
      return {
        icon:     '🥅',
        severity: 'opportunity',
        title:    I18N.t('ui.frames.oppKeeperShaky.title'),
        text:     I18N.t('ui.frames.oppKeeperShaky.text', { opp: ctx.oppName }),
        effect:   I18N.t('ui.frames.oppKeeperShaky.effect'),
        hint:     I18N.t('ui.frames.oppKeeperShaky.hint')
      };
    },
    autoEffect(match, ctx) {
      match.buffLayers.push({
        source: 'frame:opp_keeper_shaky',
        range:  [match.round, match.round],
        stats:  { offense: 10, composure: 4 },
        special: null
      });
      // Dampen their next save via the existing sticky saveBonus key —
      // negative value reduces their save chance.
      match.saveBonusOpp = (match.saveBonusOpp || 0) - 8;
    }
  },

  // Opp defense overloaded — we've had many buildups succeed, they're
  // stretched. Read: hit them with a Combo while their shape is broken.
  {
    id: 'opp_defense_stretched',
    window: [2, 5],
    priority: 5,
    fireChance: 0.50,
    condition(match) {
      const s = match.stats || {};
      // 3+ successful buildups AND score tied or leading — we've been
      // dictating but haven't fully broken through yet.
      return (s.myBuildupsSuccess || 0) >= 3 && match.scoreMe >= match.scoreOpp;
    },
    resolveContext(match) {
      return { oppName: match.opp?.name || '' };
    },
    frame(match, ctx) {
      return {
        icon:     '🧱',
        severity: 'opportunity',
        title:    I18N.t('ui.frames.oppDefenseStretched.title'),
        text:     I18N.t('ui.frames.oppDefenseStretched.text', { opp: ctx.oppName }),
        effect:   I18N.t('ui.frames.oppDefenseStretched.effect'),
        hint:     I18N.t('ui.frames.oppDefenseStretched.hint')
      };
    },
    autoEffect(match, ctx) {
      match.buffLayers.push({
        source: 'frame:opp_defense_stretched',
        range:  [match.round, match.round],
        stats:  { offense: 8, vision: 6 },
        special: null
      });
      // Their next buildup is harder (they've lost shape), so we get a
      // cleaner opportunity against their build. The engine reads
      // _oppBuildupPenalty as the malus magnitude (engine.js:2245); the
      // _oppBuildupPenaltyRounds counter only controls HOW LONG the
      // penalty lasts. Setting just the counter without the magnitude —
      // as this frame used to — was a silent no-op: the advertised
      // "their next buildup is harder" effect was dead code.
      match._oppBuildupPenalty       = Math.max(match._oppBuildupPenalty || 0, 0.15);
      match._oppBuildupPenaltyRounds = Math.max(match._oppBuildupPenaltyRounds || 0, 1);
    }
  },

  {
    // Critical condition — any starter drops below 25 condition, fires
    // a red-pulsing danger frame. Points the player at Breather/Rotation
    // and carries a small team-wide penalty until addressed.
    ...
    condition(match) {
      const squad = match.squad || [];
      return squad.some(p => typeof p.condition === 'number' && p.condition < 25);
    },
    resolveContext(match) {
      const squad = match.squad || [];
      const worst = squad
        .filter(p => typeof p.condition === 'number')
        .reduce((a, b) => (a.condition < b.condition ? a : b));
      return { player: worst };
    },
    frame(match, ctx) {
      const name = ctx.player?.name || ctx.player?.role || '';
      return {
        icon:     '🔥',
        severity: 'danger',
        title:    I18N.t('ui.frames.conditionCritical.title') || 'CRITICAL FATIGUE',
        text:     (I18N.t('ui.frames.conditionCritical.text') || '{name} is running on fumes.').replace('{name}', name),
        effect:   I18N.t('ui.frames.conditionCritical.effect') || '−2 CMP team-wide · stats heavily penalized',
        hint:     I18N.t('ui.frames.conditionCritical.hint') || 'Breather or Rotation NOW.'
      };
    },
    autoEffect(match, ctx) {
      match.buffLayers.push({
        source: 'frame:condition_critical',
        range:  [match.round, match.round],
        stats:  { composure: -2 },
        special: null
      });
...
// ─── MATCH_EVENTS — micro-events with 1-2 round reaction windows ─────────
// Lighter than FRAME_EVENTS: they don't take over the card-phase header,
// they slot into the log as "opportunity/threat" beats and tag the match
// with an active-effect. The player's card play during the window either
// cashes in the bonus or watches it expire.
//
// Each event:
//   - trigger(match, state): when to fire (e.g. after save, after miss)
//   - windowRounds: how long the opportunity lasts
//   - onFire(match): applies a transient match flag (e.g. _cornerPending)
//   - onExpire(match): called at end of window if unused
//   - log strings driven via ui.matchEvents.*
// The resolve logic lives in the engine / playCard bonus checks —
// flags are READ by card resolution, not processed here.
//
// Budget: 5 match-events per match max, 2-round cooldown between events
// so the log doesn't get spammed.
// ────────────────────────────────────────────────────────────────────────

const MATCH_EVENTS = [
  {
    id: 'corner_kick',
    // Fires after a missed shot — the ball went wide/over, ref calls a
    // corner. Next round any setup or combo card gets +35% direct-goal
    // chance because the set-piece favors us.
    trigger(match) {
      return match._justMissed && (match.round || 0) < 6;
    },
    windowRounds: 1,
    priority: 5,
    onFire(match) {
      match._cornerPending = true;
      match._cornerWindow = (match.round || 0) + 1;
    },
    onExpire(match) {
      match._cornerPending = false;
    },
    logKey: 'ui.matchEvents.cornerKick'
  },
  {
    id: 'counter_press_chance',
    // Fires after a save — keeper caught it clean, team can counter-press
    // the restart. Next round press / counter cards get +50% effect.
    trigger(match) {
      return match._justSaved && (match.round || 0) < 6;
    },
    windowRounds: 1,
    priority: 6,
    onFire(match) {
      match._counterPressWindow = (match.round || 0) + 1;
    },
    onExpire(match) {
      match._counterPressWindow = 0;
    },
    logKey: 'ui.matchEvents.counterPressChance'
  },
  {
    id: 'one_on_one',
    trigger(match) {
      ...
      if (mm < 40 || r < 3 || r >= 6) return false;
      if (!match._cardLaneOpen) return false;
      const st = (match.squad || []).find(p => p.role === 'ST');
      if (!st) return false;
      const hasComposure = (st.stats?.composure || 0) >= 60
        || (st.traits || []).some(t => typeof t === 'string' && t.includes('composure'));
      return hasComposure;
    },
    windowRounds: 1,
    priority: 8,
    onFire(match) {
      match._oneOnOnePending = true;
      match._oneOnOneWindow = (match.round || 0) + 1;
    },
    onExpire(match) {
      match._oneOnOnePending = false;
    },
    logKey: 'ui.matchEvents.oneOnOne'
  },
  {
    id: 'injury_scare',
    // Fires when a starter's condition drops below 40 mid-match.
    // 1-round window. If player plays Breather, Rotation, or Second Wind
    // on the target, the injury is averted. Otherwise next round the
    // player permanently loses 5 base stat for rest of match.
    trigger(match) {
      const r = match.round || 0;
      if (r < 3 || r >= 6) return false;
      if (match._injuryScareTarget) return false;   // already fired
      const squad = match.squad || [];
      for (const p of squad) {
        if (typeof p.condition === 'number' && p.condition < 40 && p.role !== 'TW') {
          match._injuryScareTargetId = p.id;
          return true;
        }
      }
      return false;
    ...
    priority: 9,
    onFire(match) {
      // v52.2 cleanup: `_injuryScareWindow` write removed — the value
      // was never read anywhere; cross-event cooldown is handled by the
      // framework's _lastMatchEventRound (see checkMatchEvents), not a
      // per-event window var. `_injuryScareTargetId` was the scratch
      // value set in trigger(); we consume it here and clear it to
      // avoid stale state if the match ends mid-window.
      match._injuryScareTarget = match._injuryScareTargetId;
      match._injuryScareTargetId = null;
    },
    onExpire(match) {
      // If window expires without response, apply the stat loss permanently.
      const targetId = match._injuryScareTarget;
      if (!targetId) return;
      const p = (match.squad || []).find(x => x.id === targetId);
      if (p && p.stats) {
        const stat = (p.role === 'ST' || p.role === 'LF') ? 'offense'
                   : (p.role === 'VT' || p.role === 'TW') ? 'defense'
                   : 'tempo';
        p.stats[stat] = Math.max(0, (p.stats[stat] || 50) - 5);
        match.buffLayers.push({
          source: 'event:injury',
          range: [match.round, 6],
          stats: { [stat]: 0 },   // log only, the base stat is already reduced
          special: null
        });
      }
      match._injuryScareTarget = null;
    },
    logKey: 'ui.matchEvents.injuryScare'
  },
  {
    id: 'yellow_card_threat',
    // Fires when a VT has made 2+ stops in the match AND opp momentum
    // is rising (mm <= -20 means opp pressure). Next round, if player
    // plays Desperate Foul or Tight Shape, VT absorbs a hit but gets
    // yellow (cosmetic for now; could be mechanical later). Otherwise
    // opp breaks through for likely conceded goal.
    trigger(match) {
      const r = match.round || 0;
      if (r < 2 || r >= 6) return false;
      if (match._yellowCardWindow) return false;
      const mm = match.matchMomentum || 0;
      if (mm > -15) return false;
      const vt = (match.squad || []).find(p => p.role === 'VT');
      if (!vt) return false;
      const stops = vt._matchStats?.defendedAttacks || vt._matchStats?.stops || 0;
      return stops >= 2;
    },
    windowRounds: 1,
    priority: 7,
    onFire(match) {
      match._yellowCardPending = true;
      match._yellowCardWindow = (match.round || 0) + 1;
    },
    onExpire(match) {
      match._yellowCardPending = false;
      // v52.2 — also clear the re-trigger window flag. Without this the
      // event could only fire ONCE per match because trigger() gates on
      // `_yellowCardWindow` being falsy. The window counter is
      // round-based, and since the round is already well past winEnd by
      // the time onExpire runs, a stale flag never re-appears naturally.
      // Consistent with counter_press_chance's explicit clear.
      match._yellowCardWindow = 0;
    },
    logKey: 'ui.matchEvents.yellowCardThreat'
  }
];

// Check for micro-events at round start. Parallel to checkFrameEvents,
// uses its own budget counter (_matchEventsThisMatch) and cooldown
// tracker (_lastMatchEventRound).
function checkMatchEvents(match, state, onEvent) {
  if (!match._matchEventsThisMatch) match._matchEventsThisMatch = 0;
  if (match._matchEventsThisMatch >= 5) return;
  const r = match.round || 1;
  const lastFire = match._lastMatchEventRound || 0;
  if (lastFire && (r - lastFire) < 2) return;   // 2-round cooldown

  // Also expire last round's event window if it's closing this round.
  for (const ev of MATCH_EVENTS) {
    if (match._activeMatchEvent?.id === ev.id) {
      const winEnd = match._activeMatchEvent.endRound;
      if (r > winEnd) {
        try { ev.onExpire(match); } catch (e) {
          // v52.2 — was a silent swallow. Keep the framework running if
          // a single event's cleanup throws, but surface the problem in
          // dev consoles so regressions are visible instead of hidden.
          if (typeof console !== 'undefined') console.warn('[match-event onExpire]', ev.id, e?.message || e);
        }
        match._activeMatchEvent = null;
      }
    }
  }
  ...
  const candidates = [];
  for (const ev of MATCH_EVENTS) {
    try { if (!ev.trigger(match, state)) continue; } catch (e) {
      if (typeof console !== 'undefined') console.warn('[match-event trigger]', ev.id, e?.message || e);
      continue;
    }
    candidates.push(ev);
  }
  ...
  const total = candidates.reduce((s, ev) => s + (ev.priority || 1), 0);
  ...
  try { selected.onFire(match); } catch (e) {
    if (typeof console !== 'undefined') console.warn('[match-event onFire]', selected.id, e?.message || e);
  }
  match._activeMatchEvent = {
    id: selected.id,
    startRound: r,
    endRound: r + (selected.windowRounds || 1) - 1
  };
  match._matchEventsThisMatch++;
  match._lastMatchEventRound = r;

  // Consume trigger flags so they don't re-fire next round.
  match._justMissed = false;
  match._justSaved = false;

  // Emit a log line describing the opportunity.
  if (onEvent && selected.logKey && window.I18N) {
    const picked = window.I18N.pickText(selected.logKey);
    const msg = (picked && picked !== selected.logKey) ? picked : selected.id;
    onEvent({ type: 'log', cls: 'match-event', msg: `${msg}` });
  }
}

if (typeof window !== 'undefined') {
  window.MATCH_EVENTS = MATCH_EVENTS;
  window.checkMatchEvents = checkMatchEvents;
}

// Pick and fire at most one frame per round. Sets match._activeFrame for
// the UI to render, and calls autoEffect to apply mechanics. Never shows
// a modal. Fire-and-forget, no await.
function checkFrameEvents(match, state) {
  if (!match._firedFrames)     match._firedFrames = new Set();
  if (!match._framesThisMatch) match._framesThisMatch = 0;
  if (match._framesThisMatch >= 4) return;

  // Clear last round's frame so it doesn't bleed through.
  match._activeFrame = null;

  const r = match.round || 1;
  const candidates = [];
  for (const ev of FRAME_EVENTS) {
    if (r < ev.window[0] || r > ev.window[1]) continue;
    // Stackable frames may re-fire, but only at level 2 max to avoid
    // runaway. Non-stackable frames lock after one fire.
    if (match._firedFrames.has(ev.id) && !ev.stackable) continue;
    if (ev.stackable && (match._frameLevel?.[ev.id] || 0) >= 2) continue;
    // Stackable re-fires also need at least 1 round gap — otherwise
    // the same frame fires twice on consecutive rounds which feels
    // unnatural. `_frameLastFireRound` tracks this.
    const lastFire = match._frameLastFireRound?.[ev.id];
    if (ev.stackable && match._firedFrames.has(ev.id) && lastFire != null && (r - lastFire) < 2) continue;
    try {
      if (!ev.condition(match, state)) continue;
    } catch (_) { continue; }
    candidates.push(ev);
  }
  if (!candidates.length) return;

  const eligible = candidates.filter(ev => rand() < (ev.fireChance || 0.5));
  if (!eligible.length) return;

  // Weighted pick by priority.
  const total = eligible.reduce((s, ev) => s + Math.max(1, ev.priority || 1), 0);
  let roll = rand() * total;
  let selected = eligible[0];
  for (const ev of eligible) {
    roll -= Math.max(1, ev.priority || 1);
    if (roll <= 0) { selected = ev; break; }
  }

  const ctx = selected.resolveContext ? selected.resolveContext(match, state) : {};
  // Track stacking level — 1 on first fire, 2 on re-fire. Frame
  // autoEffect reads ctx._frameLevel to decide stat strength.
  if (!match._frameLevel) match._frameLevel = {};
  if (!match._frameLastFireRound) match._frameLastFireRound = {};
  match._frameLevel[selected.id] = (match._frameLevel[selected.id] || 0) + 1;
  match._frameLastFireRound[selected.id] = r;
  ctx._frameLevel = match._frameLevel[selected.id];
  const frameData = selected.frame(match, ctx);

  try { selected.autoEffect(match, ctx); } catch (_) {}
  ...
  match._firedFrames.add(selected.id);
  match._framesThisMatch++;
  // Run-level total for the post-run stats panel. Incremented per
  // firing, not per unique frame — captures density of situations
  // across the whole run.
  if (window.state) {
    window.state._runFramesFired = (window.state._runFramesFired || 0) + 1;
  }
}
...
function _previewDecisionStats(match, decision, phase, state) {
  ...
  let fitMultValue = 1;
  let fitMisfitKey = null;  // i18n key of the concrete misfit reason (for tooltip)
  ...
  if (phase === 'kickoff' && typeof TACTIC_FIT !== 'undefined' && TACTIC_FIT[decisionId]) {
    ...
    if (isFit && !oppBreached) {
      ...
      // Capture the misfit reason so the UI can surface WHY a tactic
      // is risky (via tooltip on the badge). Without this, players see
      // "RISKY" without context and can't learn what to change.
      fitMisfitKey = fitDef.misfitKey || null;
      for (const key of Object.keys(stats)) {
  ...
  return { stats, notes, fitStatus, fitMultValue, fitMisfitKey, synergyStatus, synergyMultValue };
}
...
function decorateOptionsForDisplay(options, match, phase, state) {
  return (options || []).map(opt => {
    ...
    if (phase === 'kickoff' || phase === 'halftime' || phase === 'final') {
      ...
      // Tooltips explain WHY — without them "RISKY" was a mystery label.
      const badges = [];
      let cornerBadge = null;
      if (preview.fitStatus === 'fit') {
        cornerBadge = {
          text: I18N.t('ui.optionBadges.fitsSquad'),
          kind: 'good',
          tooltip: I18N.t('ui.optionBadges.fitsSquadTooltip')
            || 'This tactic suits your squad — bonus effect applied.'
        };
      } else if (preview.fitStatus === 'misfit') {
        // Prefer the specific misfit reason (e.g. "no pace for aggressive
        // press") over a generic "risky" blurb. Fall back to generic if
        // the tactic has no misfitKey declared.
        const misfitText = preview.fitMisfitKey
          ? I18N.t(preview.fitMisfitKey).replace(/^\s*⚠\s*/, '').trim()
          : (I18N.t('ui.optionBadges.riskyTooltip')
             || 'This tactic does not suit your squad — reduced effect.');
        cornerBadge = {
          text: I18N.t('ui.optionBadges.risky'),
          kind: 'risky',
          tooltip: misfitText
        };
      }
      if (preview.synergyStatus === 'synergy') {
        badges.push({
          text: I18N.t('ui.optionBadges.synergy', { mult: preview.synergyMultValue.toFixed(2) }),
          kind: 'good',
          tooltip: I18N.t('ui.optionBadges.synergyTooltip')
            || 'Synergizes with your previous decisions this match.'
        });
      } else if (preview.synergyStatus === 'conflict') {
        badges.push({
          text: I18N.t('ui.optionBadges.conflict', { mult: preview.synergyMultValue.toFixed(2) }),
          kind: 'risky',
          tooltip: I18N.t('ui.optionBadges.conflictTooltip')
            || 'Conflicts with your previous decisions — reduced effect.'
        });
      }
  ...
  for (const incoming of bench) {
    ...
    options.push({
      ...
      apply(m, ctx) {
        ...
        // v0.38 — Force a UI refresh so the match-footer stat bars and
        // the player-pulse dot row reflect the new lineup immediately.
        // Previously the sub mutated m.squad / lineupIds but no render
        // was triggered; the footer kept showing the OUTGOING player's
        // stats until the next natural update tick (score change or
        // round start). Players reported "match-pulse doesn't react to
        // substitutions" — this was the reason.
        // v0.46 — replacePulseTile zusätzlich: das alte Pulse-Tile war
        // an outgoing.id gebunden und wurde nie ersetzt. Kein Namens-
        // wechsel, kein Rollen-Update — der Ausgewechselte stand noch
        // namentlich auf dem Platz. Jetzt wird das Tile in-place
        // ausgetauscht.
        if (window.UI) {
          try {
            if (typeof UI.replacePulseTile === 'function') UI.replacePulseTile(outgoing.id, incoming, m);
            if (typeof UI.renderMatchFooter === 'function') UI.renderMatchFooter(m.squad, m.opp);
            if (typeof UI.updatePulseStats    === 'function') UI.updatePulseStats(m.squad);
          } catch (_) { /* UI-fehler darf den sub nicht killen */ }
        }
...
async function checkSituativeEvents(match, onEvent, state) {
  ...
  // Round-counter decrement runs UNCONDITIONALLY. Previously this block
  // lived at the bottom of this function, after the cards-enabled early
  // return — which meant timed effects in card-mode never expired. Today
  // only one card-mode setter uses this machinery (opp_defense_stretched
  // frame), but the coupling was fragile: any future frame / match-event
  // that sets a *Rounds counter would silently keep its effect alive
  // forever. Keeping the decrement here, before any return, makes the
  // contract explicit: "every round, first we tick down timed effects,
  // then we maybe fire a new event."
  tickRoundCounters(match);

  // Cards on? Use inline frames instead of modal situations.
  // Frames auto-apply their effect and set match._activeFrame; the UI
  // renders the banner atop the card panel. No modal interrupts the flow.
  const CFG = window.CONFIG || window.KL?.config?.CONFIG;
  if (CFG && CFG.cardsEnabled) {
    if (typeof checkFrameEvents === 'function') {
      try { checkFrameEvents(match, state); } catch (e) {
        if (typeof console !== 'undefined') console.warn('[checkFrameEvents]', e?.message || e);
      }
    }
    // Micro-events parallel to frames — separate budget, 2-round
    // cooldown. Fires on trigger flags set by engine events (miss,
    // save, momentum threshold etc).
    if (typeof window.checkMatchEvents === 'function') {
      try { window.checkMatchEvents(match, state, (ev) => {
        if (ev?.type === 'log' && match.log) match.log.push(ev);
      }); } catch (e) {
        if (typeof console !== 'undefined') console.warn('[checkMatchEvents]', e?.message || e);
      }
    }
    return;
  }
...
// Decrement every *Rounds counter by one. When a counter hits 0 the
// paired magnitude value is zeroed too, so the effect cleanly stops.
// Pairing table (counter → magnitude):
//   _oppBuildupPenaltyRounds → _oppBuildupPenalty
//   _oppStrikerMalusRounds   → _oppStrikerMalus
//   _oppShotMalusRounds      → _oppShotMalus
//   _keeperZoneRounds        → _keeperZoneBonus
// When adding a new timed match flag, add the pair here — otherwise the
// effect never expires and card-mode / non-card-mode balance will drift.
//
// Ordering note (balance-relevant): this is called at the START of
// checkSituativeEvents, BEFORE any event can fire this round. Previously
// the decrement ran at the END of the function, in the same call that
// might have just set a counter — which silently ate one round of
// effect (set Rounds=2, immediately decremented to 1, effective lifespan
// was 1 round not 2). With the tick moved to the top, setters now honor
// their advertised duration: Rounds=2 means two rounds of effect, etc.
// This is a small buff to every counter currently in use (they are all
// opp-malus helpers); check that before retuning values.
function tickRoundCounters(match) {
...
// ─── Intel: Probable Frames predictor ──────────────────────────────────────
// Given a lineup and an opponent, returns the frames most likely to fire
// during the upcoming match, scored by how well their trigger conditions
// match the squad/opp shape. Hub-facing intel, not used by the engine.
//
// Frames are *situational*: they fire when a runtime condition is met
// (striker frustrated, keeper beaten shots etc.). Pre-match we can't
// know what actually happens, but we can estimate by looking at stat
// differentials, opp traits, and rough squad condition state.
function predictProbableFrames(squad, opp) {
  if (!squad || !opp) return [];

  // Intel is gated behind match 3+ — the player needs to experience the
  // base mechanics (condition, cards, frames firing organically) for a
  // couple of matches before the hub starts surfacing predictive
  // analysis. Showing intel in M1 is information overload and breaks
  // the discovery arc. The rest of the hub (scorecard, edge/threat,
  // difficulty chip) still works — just the probable-situations panel
  // stays silent until the player has lived through a few matches.
  const _matchNumGate = (window.state?.matchNumber || 0) + 1;
  if (_matchNumGate < 3) return [];

  // Squad aggregates — cheap proxies for the runtime conditions.
  const mySt = squad.find(p => p.role === 'ST');
  const myTw = squad.find(p => p.role === 'TW');
  const myVt = squad.find(p => p.role === 'VT');
  const sum = (k) => squad.reduce((s, p) => s + ((p.stats && p.stats[k]) || 0), 0);
  const avgOff = sum('offense') / (squad.length || 1);
  const avgDef = sum('defense') / (squad.length || 1);
  const avgCmp = sum('composure') / (squad.length || 1);
  const oppOff = opp.stats?.offense ?? 50;
  const oppDef = opp.stats?.defense ?? 50;
  const oppCmp = opp.stats?.composure ?? 50;

  // Score each frame 0-1 based on how plausible its trigger condition is.
  // Returns { id, likelihood }. Thresholds chosen so a clear-cut matchup
  // puts the relevant frame at 0.6-0.9, a neutral matchup at 0.2-0.4.
  const scores = [];

  // striker_frustrated: ST faces a strong defense.
  if (mySt) {
    const stOff = mySt.stats?.offense ?? avgOff;
    const gap = oppDef - stOff;
    const likelihood = Math.max(0, Math.min(1, (gap + 10) / 30));
    if (likelihood > 0.2) scores.push({ id: 'striker_frustrated', likelihood });
  }

  // keeper_in_zone: TW standing tall against real attacking pressure.
  //
  // v0.60.2 — Formel umgestellt von (twDef + oppOff - 100) / 40 auf eine
  // Differenz. Die Summen-Variante war semantisch verkehrt: ein STARKER
  // gegnerischer Stürmer hat die "dein Keeper ist in Form"-Wahrscheinlichkeit
  // hochgetrieben, und ein schwacher TW (Def 40) gegen einen starken
  // Gegner (Off 70) feuerte das Frame trotzdem (40+70-100=10, /40=0.25).
  // Inhaltlich Unsinn — und der Hauptgrund warum das Frame in fast jedem
  // Match in der "Probable Situations"-Liste auftauchte.
  //
  // Neu: Keeper-Vorteil = TW-Defense MINUS Opp-Offense + leichter Bias.
  // Triggert nur wenn der Keeper relativ zum erwarteten Druck überdurch-
  // schnittlich stark steht. Schwellwert auf 0.25 angehoben (von 0.2)
  // damit das Frame nicht schon bei marginalem Edge feuert.
  if (myTw) {
    const twDef = myTw.stats?.defense ?? avgDef;
    const score = Math.max(0, Math.min(1, (twDef - oppOff + 5) / 25));
    if (score > 0.25) scores.push({ id: 'keeper_in_zone', likelihood: score });
  }

  // hot_corridor: squad offense clearly over opp defense, lane-open moment.
  const offEdge = avgOff - oppDef;
  if (offEdge > 5) {
    scores.push({
      id: 'hot_corridor',
      likelihood: Math.min(1, offEdge / 25)
    });
  }

  // opp_star_down: how likely is it the opponent loses their rhythm during
  // the match?
  //
  // v0.60.2 — Trigger umgestellt. Bisher: feste 0.35 Likelihood, sobald der
  // Gegner irgendeinen Trait-Holder hatte. Da JEDER generierte Gegner Trait-
  // Holder hat, feuerte das Frame praktisch in jedem Match — und immer mit
  // exakt einem Dot (low). War damit kein Predictor sondern ein Default-
  // Filler in der Probable-Situations-Liste.
  //
  // Neu: Likelihood skaliert mit der gegnerischen Composure. Niedrige Opp-
  // Composure = anfälliger für Rhythmus-Verlust. Über 65 Composure feuert
  // das Frame nicht mehr — solche Gegner sind mental zu stabil als dass ein
  // Pre-Match-Predictor das verlässlich vorhersagen könnte. Frame ist
  // narrativ trotzdem ein Match-Moment-Event; der Predictor schätzt nur
  // mehr die Eintritts-Wahrscheinlichkeit, nicht die Eintritts-Garantie.
  if (oppCmp < 65) {
    const likelihood = Math.max(0, Math.min(0.9, (65 - oppCmp) / 30));
    if (likelihood > 0.2) scores.push({ id: 'opp_star_down', likelihood });
  }

  // red_card_risk: aggressive squad with low composure.
  if (avgCmp < 55) {
    scores.push({
      id: 'red_card_risk',
      likelihood: Math.min(1, (60 - avgCmp) / 20)
    });
  }

  // opp_keeper_shaky: squad has a real attacking edge → likely many SOT.
  if (offEdge > 8) {
    scores.push({
      id: 'opp_keeper_shaky',
      likelihood: Math.min(1, offEdge / 20)
    });
  }

  // opp_defense_stretched: squad tempo advantage suggests chasing opp.
  const oppTmp = opp.stats?.tempo ?? 50;
  const avgTmp = sum('tempo') / (squad.length || 1);
  if (avgTmp - oppTmp > 6) {
    scores.push({
      id: 'opp_defense_stretched',
      likelihood: Math.min(1, (avgTmp - oppTmp) / 20)
    });
  }

  // condition_critical: actual fatigue pressure in the squad, weighted
  // by season progress. Previously fired purely on match-number (>=8)
  // which wrongly predicted "CRITICAL FATIGUE" even when every starter
  // was at 80%+ (v52.1 fix). Now requires at least one real condition
  // signal: either a starter below 55% OR the squad average below 70%.
  // Likelihood scales with how bad the actual fatigue is + late-season
  // bonus so a tired squad in match 12 reads as higher risk than the
  // same numbers in match 3.
  const matchNum = (window.state?.matchNumber || 0) + 1;
  const condValues = squad
    .map(p => p.condition)
    .filter(v => typeof v === 'number');
  if (condValues.length) {
    const minCond = Math.min(...condValues);
    const avgCond = condValues.reduce((a, b) => a + b, 0) / condValues.length;
    // Severity 0..1 — based on whichever signal is worse.
    const minSev = Math.max(0, (55 - minCond) / 55);   // 0 at 55+, 1 at 0
    const avgSev = Math.max(0, (70 - avgCond) / 70);   // 0 at 70+, 1 at 0
    const severity = Math.max(minSev, avgSev);
    if (severity > 0.15) {
      // Late-season bump: fatigue becomes more likely to escalate as
      // the run goes on (cumulative loads). Caps at +0.3 at match 12+.
      const seasonBump = Math.max(0, Math.min(0.3, (matchNum - 6) * 0.04));
      scores.push({
        id: 'condition_critical',
        likelihood: Math.min(1, severity + seasonBump)
      });
    }
  }

  // Top 3 by likelihood, descending.
  scores.sort((a, b) => b.likelihood - a.likelihood);
  return scores.slice(0, 3);
}

// Card-IDs that specifically address specific frames. Used by the hub to
// tell the player "your deck has 2 counters for this match".
const FRAME_COUNTERS = {
  // striker_frustrated: swap-lane or alternate-striker coverage.
  striker_frustrated:  ['switch_lane', 'rotation', 'breather'],
  // keeper_in_zone: payoff cards multiply the saved-opportunity moment.
  keeper_in_zone:      ['counter_strike', 'bait_counter'],
  // hot_corridor: cards that consume Lane Open.
  hot_corridor:        ['through_ball', 'clinical_finish', 'killing_blow', 'overlap_run'],
  // opp_star_down: payoff aggressive play when opp is weakened.
  opp_star_down:       ['gegenpress', 'high_press_trap', 'forward_burst'],
  // red_card_risk: composure-saving cards matter.
  red_card_risk:       ['tight_shape', 'hold_the_line', 'deep_focus'],
  // opp_keeper_shaky: finishers while the keeper's rattled.
  opp_keeper_shaky:    ['stone_cold', 'masterclass', 'clinical_finish'],
  // opp_defense_stretched: combo-through-middle.
  opp_defense_stretched:['through_ball', 'overlap_run', 'flank_overload'],
  // condition_critical: condition-management cards.
  condition_critical:  ['breather', 'rotation', 'stamina_boost']
};

// Given the player's full deck contents (list of cardIds) and a predicted
// frame list, count how many cards in the deck address each frame.
function countDeckFrameCounters(deckCardIds, predictedFrames) {
  if (!Array.isArray(deckCardIds) || !Array.isArray(predictedFrames)) return [];
  const result = [];
  for (const pred of predictedFrames) {
    const counters = FRAME_COUNTERS[pred.id] || [];
    const matchCount = deckCardIds.filter(id => counters.includes(id)).length;
    result.push({
      frameId:     pred.id,
      likelihood:  pred.likelihood,
      counterCount: matchCount,
      counterCardIds: deckCardIds.filter(id => counters.includes(id))
    });
  }
  return result;
}
...
window.predictProbableFrames   = predictProbableFrames;
window.countDeckFrameCounters  = countDeckFrameCounters;
window.FRAME_COUNTERS          = FRAME_COUNTERS;
